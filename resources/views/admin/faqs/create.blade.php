            ...
            <div class="p-8">
                ...
                @if($categories->isEmpty())
                    <div class="mb-6 bg-yellow-900/50 border border-yellow-500 text-yellow-300 px-4 py-3 rounded">
                        ...
                        <p>Je moet eerst een FAQ categorie aanmaken voordat je een FAQ kunt toevoegen.</p>
                        <a href="{{ route('admin.faq-categories.create') }}" class="inline-block mt-3 px-4 py-2 bg-yellow-600 text-gray-900 rounded hover:bg-yellow-500 transition font-semibold">
                            + Maak Categorie
                        </a>
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('admin.faqs.store') }}">
                    ...
                    <div class="mb-6">
                        ...
                        <input id="question"
                               ...
                               value="{{ old('question') }}"
                               required
                               placeholder="bijv. Hoe verander ik mijn wapen?"
                               class="mt-1 block w-full rounded-md shadow-sm py-3 px-4">
                        @error('question')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="answer"
                                  ...
                                  class="mt-1 block w-full rounded-md shadow-sm py-3 px-4">{{ old('answer') }}</textarea>
                        @error('answer')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
